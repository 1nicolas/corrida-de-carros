class Game {
  ...
  start() {
    ...
    this.addSprites(fuels, 4,fuelImg,0.03);
    this.addSprites(coins, 4,coinsImg,0.1)
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      ...
      const endEndEnd = height*6-100;
      if(player.positionY > endEndEnd){
        gameState = 2;
        player.rank +=1;
  ...
  handleObstacleCollision(index){
    ...
    if(cars[index -1].collide(obstacles)){
      ...
      if(player.life>0){player.life-=179/4}
    }
    player.update()
  }
  ...
  showLife(){
    push()
    image(lifeImg, width/2-130, height-player.positionY-270,20,20);
    fill('white')
